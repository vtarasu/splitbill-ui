...
export default function ExpenseModal({ expense, members, groupId, onClose, onSaved }) {
    ...
    const save = async () => {
        if (!desc || !amount) { alert("Fill in all fields."); return; }
        ...
        if (splitStrategy === "EXACT") {
            ...
            const entries = Object.entries(exactAmounts).filter(([_, v]) => v);

            if (!entries.every(([_, v]) => parseFloat(v) > 0 && !isNaN(parseFloat(v)))) {
                alert("Amounts must be positive numbers");
                return;
            }

            const sum = entries.reduce((acc, [_, v]) => acc + parseFloat(v), 0);
        ...
        if (splitStrategy === "SHARES") {
            const entries = Object.entries(shares).filter(([_, v]) => v);
        ...
        if (splitStrategy === "EQUAL") {
            const selected = members.filter(m => selectedUsers[m.userId]);
        ...
        try {
            ...
            if (err.response?.status === 429 && err.response?.data?.code === "MAX_LIMIT_REACHED") {
    ...
    return (
        <>
            ...
            {showMaxLimitModal && (
                <Modal title="Limit Reached" onClose={() => setShowMaxLimitModal(false)}>
                    <p style={{ fontSize: 14, marginBottom: 16 }}>
                        You've reached the maximum number of expenses. Get subscription to add more and enjoy additional features!
                    </p>
                    <ModalFooter onCancel={() => setShowMaxLimitModal(false)}>
                        <PrimaryBtn label="Okay" onClick={() => setShowMaxLimitModal(false)} />
                    </ModalFooter>
                </Modal>
            )}
        </>
